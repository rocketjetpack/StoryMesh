...
export default function JobsPanel(): JSX.Element {
  ...
  // Show queued + running first; completed and failed are de-emphasized.
  const visible = (jobs ?? []).filter((j) => j.status === "running" || j.status === "queued");
...
function JobCard({ job }: { job: JobStatus }): JSX.Element {
  ...
  const progress = job.total_stages > 0 ? job.stage_index / job.total_stages : 0;
  ...
  return (
    <motion.li
      ...
      className="border-l-2 border-brass/60 pl-3 py-2"
    >
      <div className="flex items-center gap-2 mb-1">
        {job.status === "running" ? (
          <span className="ink-dot animate-ink-pulse" aria-hidden />
        ) : (
          <span className="ink-dot opacity-30" aria-hidden />
        )}
      ...
      <div className="flex items-center justify-between text-xs">
        <span className="mono text-cream/60">{stageLabel}</span>
        {job.status === "queued" && job.queue_position !== null && (
          <span className="mono text-cream/40">queue #{job.queue_position}</span>
        )}
        {job.status === "running" && (
